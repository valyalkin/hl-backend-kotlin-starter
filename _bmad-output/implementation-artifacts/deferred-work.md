...
## Deferred from: implementation of spec-3-2-otlp-trace-export-no-op-when-unconfigured (2026-09-14)

- source_spec: `_bmad-output/implementation-artifacts/spec-3-2-otlp-trace-export-no-op-when-unconfigured.md`
  summary: Add JDBC/Postgres span export so a request that only touches the database (no cache hit or miss) also produces an outbound-call span, not just the HTTP request span.
  evidence: Implementation discovery. Spring Boot 4.1.1 ships no built-in JDBC/Hibernate tracing instrumentation (confirmed empirically: `spring-boot-micrometer-tracing`, `spring-boot-micrometer-observation`, and `spring-jdbc-7.0.9` jars contain no JDBC/datasource observation classes) — HTTP request spans and Redis cache-call spans export automatically via Story 3.2's config-only wiring, but DB calls don't. The only known fix is a third-party datasource-proxy library (e.g. `net.ttddyy.observation:datasource-micrometer-spring-boot-starter`), which is not BOM-managed by Spring Boot and would need a pinned version in `gradle/libs.versions.toml`, conflicting with Story 3.2's frozen BOM-managed-only constraint. Human chose to narrow Story 3.2's scope to cache-call spans only rather than relax that constraint (see spec-3-2's Spec Change Log, 2026-09-14). Settle by evaluating a datasource-proxy dependency (and updating the BOM-managed convention for it, or accepting an explicit exception) in a dedicated follow-up story.

## Deferred from: code review of spec-3-3-structured-json-logging-with-trace-correlation (2026-09-16)

- source_spec: `_bmad-output/implementation-artifacts/spec-3-3-structured-json-logging-with-trace-correlation.md`
  summary: Production `src/main/resources/application.yaml`'s `logging.structured.format.console: ecs` is never directly verified by any test — `StructuredLoggingIT` runs against `src/test/resources/application.yaml`'s own independent, fully-shadowing copy of that key, so a regression in the main file alone (typo, accidental deletion) would silently revert production console output to human-readable while `./gradlew build` stayed green.
  evidence: verification-gap review. Confirmed Spring resolves `classpath:/application.yaml` to a single resource (the test file, which precedes the main file on the test classpath and whose own header comment states it "fully shadows the main application.yaml during tests"), not a merge of both. This is the same pre-existing, repo-wide pattern every earlier mirrored property (Story 3.1's actuator exposure list, Story 3.2's tracing sampling/endpoint keys, Story 2.7's cache-names) already carries — real for all of them, not introduced by this story. A fix needs a module-wide config-parity check (e.g., a test asserting the two YAML files' relevant keys match, or loading main's file directly in one IT) rather than a per-story patch.
